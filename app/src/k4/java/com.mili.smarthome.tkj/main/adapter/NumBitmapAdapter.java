...
public class NumBitmapAdapter extends MultiImageView.BitmapAdapter {
    ...
    public Bitmap getBitmap(int position, BitmapFactory.Options options) {
        int resId;
        if (isMask(position)) {
            resId = R.drawable.pwd_star;
        } else {
            resId = ResUtils.getImageId(mContext, "call_" + mText.charAt(position));
        }
    ...
    public int getDrawablePadding() {
        return mContext.getResources().getDimensionPixelOffset(R.dimen.dp_5);
    }
