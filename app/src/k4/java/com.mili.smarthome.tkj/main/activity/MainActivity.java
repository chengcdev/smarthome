...
public class MainActivity extends K4BaseActivity implements ICardStateListener, IDoorAlarmListener, ICallMonitorListener, View.OnClickListener, FuncCodeListener {
    ...
    private WffrScanFragment mFmWffrFace = null;
    private MegviiScanFragment mFmMegviiFace = null;
...
//    private HintView mHvHint;
    ...
    private Intent mBodyIntent;

    private MainBroadcastReceiver mReceiver;
    private boolean mFaceAddFlag = false;
    ...
    private void initFunction() {
        ...
        if (mFmCurrent instanceof WffrScanFragment || mFmCurrent instanceof MegviiScanFragment) {
            LogUtils.d(Tag + " current is WffrScanFragment, donot show mainFragment.");
            return;
        }
    ...
    private boolean parseIntent(Intent intent) {
        ...
        if (intent != null) {
            int funcCode = intent.getIntExtra(ARGS_FUNC, -1);
            LogUtils.d(Tag + " initFunction: funcCode is " + funcCode);
            if (funcCode > 0) {
                showFunction(funcCode, true);
                intent.putExtra(MainActivity.ARGS_FUNC, 0);
                return true;
            }
        }
        return false;
    ...
    public void setFragmentSize(boolean fullscreen) {
        ...
        if (fullscreen == mIsFullScreen) {
            if (mIsFirst) {
                ...
                LogUtils.d(Tag + " fullscreen is equal to the last.");
                return;
            }
        }
        if (fullscreen) {
            ...
            ViewGroup.LayoutParams params = mFrameLayout.getLayoutParams();
            params.width = mContext.getResources().getDimensionPixelOffset(R.dimen.dp_288);
            params.height = mContext.getResources().getDimensionPixelOffset(R.dimen.dp_280);
            mFrameLayout.setLayoutParams(params);
        }
        ...
        switch (funcId) {
            ...
            case FUNCTION_FACE:
                if(AppConfig.getInstance().getFaceManufacturer() == 0) {
                    if (mFmWffrFace == null) {
                        mFmWffrFace = new WffrScanFragment();
                    }
                    mFmWffrFace.setFuncCodeListener(this);
                    fragmentReplace(mFmWffrFace);
                }else if(AppConfig.getInstance().getFaceManufacturer() == 1){
                    if (mFmMegviiFace == null) {
                        mFmMegviiFace = new MegviiScanFragment();
                    }
                    mFmMegviiFace.setFuncCodeListener(this);
                    fragmentReplace(mFmMegviiFace);
                }
                break;
        }
    }
...
//        mHvHint.setVisibility(View.INVISIBLE);
    ...
    public void onCardState(int state, String roomNo) {
        ...
        // 在人脸界面只播放提示音不做文字提示
        if (mFmCurrent instanceof WffrScanFragment || mFmCurrent instanceof MegviiScanFragment) {
            LogUtils.d(Tag + " WffrScanFragment, play sound only.");
            if (state == 0) {
...
//                    mHvHint.setVisibility(View.INVISIBLE);
            ...
            public void run() {
                ...
                if (freeTime > Constant.SCREEN_BACKMAIN_TIME) {
                    ...
                    if (!(mFmCurrent instanceof MainFragment)) {
                        ...
                        // 扫码界面自动退出间隔3秒后才处理人体感应
                        if (mFmCurrent instanceof QrcodeFragment) {
    ...
    public void onFuncCode(int funcCode, int param) {
        ...
        switch (funcCode) {
            ...
            case 0xFF:
                LogUtils.d(Tag + " ======== faceFragemnet->onFuncCode =============");
                mMainHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        LogUtils.d(Tag + " ======== startScreenSaver =============");
    ...
    private void delayBodyInstruction(int funcId){
        if (funcId != FUNCTION_FACE && mFmCurrent instanceof BaseFaceFragment){
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (action) {
                case Const.Action.MAIN_DEFAULT:
                    LogUtils.e(Tag + "==== broadcast receive: MAIN_DEFAULT");
                    showFunction(FUNCTION_MAIN, true);
                    break;
                default:
                    break;
