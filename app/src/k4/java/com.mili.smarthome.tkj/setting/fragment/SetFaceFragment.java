...
public class SetFaceFragment extends K4BaseFragment implements ItemSelectorAdapter.OnItemClickListener, View.OnClickListener {
    ...
    public boolean onKeyConfirm() {
        ...
        if (mSetFunc == FACE_FUNC_CLEAR) {
            //清空人脸记录
            boolean success = FacePresenterProxy.clearFaceInfo();
            showSetHint(success);
        } else {
    ...
    private void showFaceStatus() {
        ...
        if (AppConfig.getInstance().getFaceRecognition() != 1) {
            enable = 0;
            array = mContext.getResources().getStringArray(R.array.setting_enabled2);
        } else {
            enable = 1;
            array = mContext.getResources().getStringArray(R.array.setting_face);
    ...
    private void showRtspSet() {
        FragmentProxy.getInstance().showFragment(new SetRtspFragment());
    }
