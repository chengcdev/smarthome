...
//    private LinearLayout llPage3;
    ...
    private TextView tvPhotoStranger; // 陌生人脸拍照
    ...
    private TextView tvFaceVer;// 人脸库版本
    private TextView tvSoftVer;// 软件
    private TextView tvHardVer; // 硬件版本
    private TextView tvBuildVer; // 编译版本
...
//        llPage3 = findView(R.id.ll_page3);
        ...
        tvPhotoStranger = findView(R.id.tv_photo_face_stranger);
        ...
        tvFaceVer = findView(R.id.tv_face_ver);
        tvSoftVer = findView(R.id.tv_soft_ver);
        tvHardVer = findView(R.id.tv_hard_ver);
        tvBuildVer = findView(R.id.tv_build_ver);
        tvMac = findView(R.id.tv_mac);
        tvSn = findView(R.id.tv_sn);
        ivSn = findView(R.id.iv_sn);

        TextView btnUpgrade = findView(R.id.btn_upgrade);
        if (btnUpgrade != null) {
            btnUpgrade.setOnClickListener(this);
        }

        TextView btnFaceAuth = findView(R.id.btn_face_auth);
        if (btnFaceAuth != null) {
            btnFaceAuth.setOnClickListener(this);
            btnFaceAuth.setVisibility(View.GONE);
            if (CustomVersion.VERSION_KAIWEI_WUYE) {
                btnFaceAuth.setVisibility(View.VISIBLE);
            }
        }
    ...
    private void initPage1() {
        ...
        if (AppConfig.getInstance().isFaceEnabled()) {
            ...
            tvFaceFree.setText(getString(R.string.device_info_face_num) + FacePresenterProxy.getSurplus());
        } else {
    ...
    private void initPage3() {
        ...
        //人脸开门拍照
        if (!AppConfig.getInstance().isFaceEnabled()) {
            tvPhotoFace.setVisibility(View.GONE);
        } else {
            tvPhotoFace.setVisibility(View.VISIBLE);
        ...
        //指纹开门拍照
        if (!AppConfig.getInstance().isFingerEnabled()) {
            tvPhotoFinger.setVisibility(View.GONE);
        } else {
            tvPhotoFinger.setVisibility(View.VISIBLE);
        ...
        //扫码开门拍照
        if (showQrcode()) {
            tvPhotoQrcode.setVisibility(View.VISIBLE);
            ...
            tvPhotoQrcode.setVisibility(View.GONE);
        }

        //陌生人脸拍照
        if (!AppConfig.getInstance().isFaceEnabled()) {
            tvPhotoStranger.setVisibility(View.GONE);
        } else {
            tvPhotoStranger.setVisibility(View.VISIBLE);
            if (snapParam.getFaceStrangerSnap() == 0) {
                tvPhotoStranger.setText(getString(R.string.device_info_24) + getString(R.string.setting_close));
            } else {
                tvPhotoStranger.setText(getString(R.string.device_info_24) + getString(R.string.setting_enable));
            }
        }
    }
